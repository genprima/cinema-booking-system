...
public interface UserOtpRepository extends JpaRepository<UserOtp, Long> {
    ...
    @Query("SELECT uo FROM UserOtp uo WHERE uo.user = :user AND uo.expiresAt > :now AND uo.used = false")
    Optional<UserOtp> findLatestActiveOtp(@Param("user") User user, @Param("now") Instant now);
